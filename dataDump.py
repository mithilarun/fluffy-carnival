...
for instance,bandwidthList in bandwidthDict.iteritems():
	...
	cur.execute("create table if not exists {} (currentdatetime VARCHAR(50),bandwidthusage INTEGER);".format("tenant"+str(i)))
	cur.execute("SET sql_notes=1; ")
	cur.execute("INSERT INTO {} (currentdatetime,bandwidthusage) values ('{}',{});".format("tenant"+str(i),datetime.now().strftime('%Y/%m/%d %H:%M:%S'),sum(bandwidthList)))
	db.commit()
	i=i+1	
